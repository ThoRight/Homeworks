...
public class Account{
    ...
    public boolean isfollowing(Account acc){
        for(int i=0;i<followings.size();++i){
            if(followings.get(i) == acc && followings.getNode(i).getisdeleted()==false)
                return true;
        }
        return false;
    }
    ...
    public boolean isblocked(Account acc){
        for(int i=0;i<blocks.size();++i){
            if(blocks.get(i) == acc && blocks.getNode(i).getisdeleted()==false)
                return true;
        }
        return false;
    }
    ...
    /**
     * make account unfollow the other acc.
     * @param acc the account that we want to unfollow.
     */
    public void unfollow(Account acc){
        ...
        if(!isLogin()){
            ...
            boolean check = followings.remove(acc);
            if(check == false){
    ...
    /**
     * after unfollowing, it removes the same information to otherside.
     * it help us make two-sided operations.
     */
    ...
    /**
     * make account unblock other accounts.
     * @param acc the account that we want to unblock.
     */
    public void unblock(Account acc){
        if(!isLogin()){
            ...
            boolean check = blocks.remove(acc);
            if(check == false){
    ...
    public String toString(){
        return "UserID: " + accountId + "\nUsername: " + username + "\nLocation: " + location
                + "\nBirth Date: " + birthdate + "\n you are following " + followingNumber
                + " account(s) " + "and have "+ followerNumber + " follower(s)";
    }
    ...
    public void viewProfile(Account acc){
        if(!isLogin()){
            ...
            for(int i=0;i<acc.followers.size();++i){
                if(acc.followers.getNode(i).getisdeleted()==false)
                    System.out.printf("%s, ", acc.followers.get(i).getUsername());
            }
            System.out.printf("\n%s is following: ", acc.getUsername());
            for(int i=0;i<acc.followings.size();++i){
                if(acc.followings.getNode(i).getisdeleted()==false)
    ...
    public void viewownProfile(){
        ...
        for(int i=0;i<followers.size();++i){
            if(followers.getNode(i).getisdeleted()==false)
                System.out.printf("%s, ", followers.get(i).getUsername());
        }
        System.out.printf("\nYou are following: ");
        for(int i=0;i<followings.size();++i){
            if(followings.getNode(i).getisdeleted()==false)
                System.out.printf("%s, ", followings.get(i).getUsername());
        }
        System.out.printf("\nBlocked account(s): ");
        for(int i=0;i<blocks.size();++i){
            if(blocks.getNode(i).getisdeleted()==false)
    ...
    public boolean isalreadyfollow(Account acc){
        for(int i=0;i<followings.size();++i){
            if(this.followings.get(i).getaccountId()==acc.getaccountId() && this.followings.getNode(i).getisdeleted()==false)
                return true;
        }
        return false;
    }
     ...
     */
    public void viewInteractions(LDLinkedList<Account> accs, Account target){
        if(!isLogin()){
            ...
            for(int i=0;i<target.getpostNumber();++i){
                ...
                if(target.posts.get(i).getlikeNumber()==0){
                    ...
                    for(int j=0;j<target.posts.get(i).getlikes().size();++j){
                        if(target.posts.get(i).getlikes().getNode(j).getisdeleted()==false)
                ...
                if(target.posts.get(i).getcommentNumber()==0){
                    ...
                    for(int j=0;j<target.posts.get(i).getcomments().size();++j){
                        int count=1;
                        if(target.posts.get(i).getcomments().getNode(j).getisdeleted()==false){
                            System.out.printf(" Comment %d: '%s' said '%s'\n", count, findaccname(accs,target.posts.get(i).getonecomment(j).getaccountId()), target.posts.get(i).getonecomment(j)); 
                            ++count;
                        }
                    }
                    System.out.printf("\n");
     ...
     */
    public void viewInbox(LDLinkedList<Account> accs){
        if(!isLogin()){
    ...
    /**
     * to find accounts' usernames in accounts LDLinkedList.
     * @param accs the LDLinkedList that keeps all registered account.
     * @param id the id number that we want to find account's username.
     * @return account's username.
     */
    public String findaccname(LDLinkedList<Account> accs, int id){
     ...
     * @param accs[] to print information with accounts' usernames.
     */
    public void viewOutbox(LDLinkedList<Account> accs){
        if(!isLogin()){
