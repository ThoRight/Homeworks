...
public class TestClass2{
    ...
    //Because static is forbidden, i check individually wheter there is same username or not.
        ...
        accs.get(2).login();
        Post post1sibelgulmez = new Post(1,accs.get(2),"I like Java.");
        Post post2sibelgulmez = new Post(2,accs.get(2),"Java the coffee.");
        System.out.printf("Sharing two posts...\n");
        accs.get(2).sharePost(post1sibelgulmez);
        accs.get(2).sharePost(post2sibelgulmez);
        ...
        Post post1 = new Post(3,accs.get(1),"Post1");
        accs.get(1).sharePost(post1);
        System.out.printf("Shares another post\n");
        Post post2 = new Post(4,accs.get(1),"Post");
        accs.get(1).sharePost(post2);
        System.out.printf("Logging out from account '%s'\n", accs.get(1).getUsername());
        ...
        accs.get(2).likePost(accs.get(viewedPostsid), like4);
        System.out.printf("Logging out from account '%s'\n", accs.get(2).getUsername());
        ...
        accs.get(3).login();
        accs.get(3).viewProfile(accs.get(1));
        viewedPostsid = 1;
        Comment comment2 = new Comment(2,3,4,"Nice!");
        System.out.printf("Adding a comment on a post of %s\n", accs.get(viewedPostsid).getUsername());
        accs.get(3).addComment(accs.get(viewedPostsid), comment2);
        System.out.printf("Sends a message to %s\n", accs.get(1).getUsername());
        Message message2 = new Message(2,3,1,"Hello!");
        accs.get(3).setisviewingProfile(false);
        accs.get(3).sendMessage(accs.get(message2.getReceiverId()),message2);
        System.out.printf("Logging out from account '%s'\n", accs.get(3).getUsername());
        accs.get(3).logout();

        System.out.printf("%s logs in.\n", accs.get(1).getUsername());
        accs.get(1).login();
        accs.get(1).viewProfile(accs.get(1));
        accs.get(1).setisviewingProfile(false);
        accs.get(1).viewInbox(accs);
        System.out.printf("Ending scenario2 ...........\n------\n\n\n\n");
