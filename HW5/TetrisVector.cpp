...
int TetrisVector:: getlc()const{
    return lc;
}

int TetrisVector:: getrc()const{
    return rc;
}

void TetrisVector:: setlc(int value){
    lc = value;
}

void TetrisVector:: setrc(int value){
    rc = value;
}

// to determine wheter it is 'I' tetromino and its position is vertical or not
bool TetrisVector:: isI(const Tetromino &piece, const char &direction)const{
    if(direction == 'R'){
...
bool TetrisVector:: check_index(const char &direction, const int &count)const{
    if(direction == 'R'){
...
TetrisVector& TetrisVector:: operator+=(const Tetromino& oth){
    int i,j,x,y;
